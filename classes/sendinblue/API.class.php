...
class API
{
     ...
     * @return  object  Sendinblue API object
     */
    public static function getInstance()
    {
        static $api = NULL;
        if ($api === NULL) {
            $api = new self(Config::get('sendinblue_api_key'));
        }
        return $api;
    }
    ...
    public function listMembers($list_id, $opts=array())
    {
        ...
        //echo $url;die;
        $response = $this->get("contacts/lists/$list_id/contacts?$url");
        if (is_array($response) && isset($response['contacts'])) {
            foreach ($response['contacts'] as $resp) {
                $retval[$resp['id']] = new SubscriberInfo;
